...
public class eComStepDefinition extends Utils {
	...
	{
		Assert.assertFalse(orderDetails.getData().getProductOrderedId().isEmpty());
		Assert.assertNotNull(verifyJsonfeildIsNotNullInResponseBody(response,fieldName),"The field "+fieldName+" is NULL");;
		
	}

	@Given("verify that orderById in the response body is NOT EMPTY")
	public void verify_that_order_by_id_in_the_response_body_is_not_empty() 
	{
		orderDetails = response.as(ViewOrderDetails.class);
		Assert.assertNotNull(orderDetails.getData().getOrderById(), "OrderById is NULL");
		Assert.assertFalse(orderDetails.getData().getOrderById().isEmpty());
	}
	
	@Given("verify that orderBy in the response body is NOT EMPTY")
	public void verify_that_order_by_in_the_response_body_is_not_empty() 
	{
		Assert.assertNotNull(orderDetails.getData().getOrderBy(), "OrderBy is NULL");
		Assert.assertFalse(orderDetails.getData().getOrderBy().isEmpty());
	}
	
	@Given("The session token and productID to be deleted")
	public void the_session_token_and_product_id_to_be_deleted() throws IOException 
	{
		req = given().spec(requestSpecificationForEcom()).header("Authorization",sessionToken).pathParam("id", productID);    
	}
